...
var createGetter = require('./create-expression').createGetter;
var createSetter = require('./create-expression').createSetter;
...
var methods = {
  $watch: function(expr, fn, options){
    ...
    if (get === undefined && rawExpr) {
      get = createGetter(rawExpr);
    }


    var { holderId, isList, track, extra, ignoreHolderUpdate } = expr;
    ...
    var watcher = {
      ...
      isList,
      track,
      ignoreHolderUpdate, // for r-style, not updating in singleWatch but in directive watchers
      once: once, 
      force: options.force,
    ...
    // maybe exists initial value from options.last
    // or calculated when option.sync is true
    // may not trigger changes after the first calculation in following dirty check
    // so, we should sync data once
    if ( holderId != undefined ) {
      this._updateMPHolders(watcher.last, {
        id: holderId,
        extra: extra,
        isList: isList,
        track: track
  ...
  _checkSingleWatch: function(watcher){
    ...
    if(dirty && !watcher.test){
      ...
      var { holderId, extra, isList, track, ignoreHolderUpdate } = watcher;
      if (!ignoreHolderUpdate && holderId != undefined) {
        this._updateMPHolders(now, {
          id: holderId,
          extra: extra,
          isList: isList,
          track: track
  ...
  $get: function(expr, detect)  {
    if(detect && typeof expr === 'string') {
      return expr;
    }
    var get = this.$expression(expr).get;
    if (!get) {
      get = createGetter(expr);
    }
    return get(this);
  },
  $update: function() {
