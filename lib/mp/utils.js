...
function getIndexMap(extra) {
  const { __listInfo__ = [] } = extra;
  const indexMap = __listInfo__.reduce((res, info) => {
    const { indexName, keyName } = info;
    if (extra[indexName] !== undefined) {
      res[indexName] = extra[indexName];
    }
    if(extra[keyName] !== undefined) {
      res[keyName] = extra[keyName];
    }
    return res;
  }, {});
