...
walkers.component = function(ast, options){
  ...
  var definition = {
    ...
    _componentId: getComponentId(ast, options),  // for mp: componentId in a rgl
    _body: {
      ctx: this,
      ast: ast.children
    }
  ...
  function getComponentId(ast, options) {
    var extra = options.extra;
    var listInfo = extra.listInfo;
    if (listInfo) {
      var listItemId = [];
      for (var i = listInfo.length - 1; i >= 0; --i) {
        listItemId.push(extra[listInfo[i].indexName || listInfo[i].keyName])
      }
      return ast.componentId + '-' + listItemId.join('-');
    }
    return ast.componentId + '';
  }
...
walkers.attribute = function(ast ,options){
  ...
  if(directive && directive.link){
    var extra = {
      ...
      eventId: options.extra && options.extra.eventId
    }
