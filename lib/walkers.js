...
walkers.element = function(ast, options){
  ...
  if (ast.staticClass || ast.staticClassHolderIds) {
    extra.staticClass = ast.staticClass
    extra.staticClassHolderIds = ast.staticClassHolderIds
  }
  extra.classId = ast.classId
  ...
  if(directive && directive.link){
    ...
    if(value.type === 'expression' && value.get){
      this.$watch(value, function(nvalue, old){
        ...
        if (name === 'class') {
          this._updateMPHolders(nvalue, {
            key: '__holders',
            id: `class${options.extra.classId}`,
            extra: options.extra
          });
        }
      }, OPTIONS.STABLE_INIT);
