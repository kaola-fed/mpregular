...
walkers.component = function(ast, options){
  ...
  for(var i = 0, len = attrs.length; i < len; i++){
    ...
    var value = this._touchExpr(attr.holder || attr.value);
...
walkers.attribute = function(ast, options){
  ...
  value = this._touchExpr(attr.holder || value);

  if (/^bind\w+$/.test(name) && eventId) {
    this._addMPEventHandler(eventId, name.slice(4), value)
    return
  }
  ...
  if(directive && directive.link){
    ...
    if(value.type === 'expression' && value.get){
      this.$watch(value, function(nvalue, old){
        ...
        _.updateHolders.call(this, nvalue, attr.holder.id, options.extra)
      }, OPTIONS.STABLE_INIT);
    }else{
      if(_.isBooleanAttr(name)){
        if (attr.holder) {
          _.updateHolders.call(this, true, attr.holder.id, options.extra)
        }
        // dom.attr(element, name, true);
      }else{
        if (attr.holder) {
          _.updateHolders.call(this, value, attr.holder.id, options.extra)
        }
