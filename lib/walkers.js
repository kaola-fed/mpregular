...
walkers.element = function(ast, options){
  ...
  var rClassId = getRClassId(ast, options)
  if (ast.staticClass) {
      extra.staticClass = ast.staticClass
  }

  var destroies = walkAttributes.call(this, attrs, element, extra, eventId, rhtmlId, rClassId);
...
function walkAttributes(attrs, element, extra, eventId, rhtmlId, rClassId) {
  var bindings = []
  for(var i = 0, len = attrs.length; i < len; i++){
...
function getRClassId(ast, options) {
  return (ast.rClassId + '') || '0';
}
