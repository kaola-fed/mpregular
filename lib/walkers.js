  ...
  if(directive && directive.link){
    ...
    if(value.type === 'expression' && value.get){
      this.$watch(value, function(nvalue, old){
        ...
        let staticClassArr = options.extra.staticClass.split(' ')
        let dynamicClassArr = nvalue.split(' ')
        let final = [...new Set(staticClassArr.concat(dynamicClassArr))].join(' ')

        updateClass.call(this, final, name, options)
      }, OPTIONS.STABLE_INIT);
    }else{
      if(_.isBooleanAttr(name)){
        // dom.attr(element, name, true);
      }else{
        updateClass.call(this, options.extra.staticClass, name, options)
...
function updateClass(value, name, options) {
